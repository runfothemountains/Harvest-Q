...
const LANG = {
  ...
  hi: {
    ...
    'mode-all':'सभी','mode-sale':'बिक्री के लिए','mode-trade':'विनिमय के लिए','mode-бoth':'दोनों',
  ...
  // simple stubs so RU / CN can be added later to the dropdown
  ru: {
    'tab-home':'Домой','tab-farmers':'Фермеры','tab-consumers':'Покупатели','tab-laws':'Законы',
    'tab-medical':'Медицина','tab-trade':'Бартер','tab-ai':'ИИ','tab-grants':'Гранты',
    'header-sub':'Свежие • Местные • Надёжные',
    'refresh':'Обновить','all-cities':'Все города'
  },
  zh: {
    'tab-home':'首页','tab-farmers':'农户','tab-consumers':'消费者','tab-laws':'法规',
    'tab-medical':'健康','tab-trade':'以物易物','tab-ai':'AI','tab-grants':'资助',
    'header-sub':'新鲜 • 本地 • 可信',
    'refresh':'刷新','all-cities':'所有城市'
  }
};
...
// 🔁 UPDATED: make sure we ignore old "quickstart" and fall back to home
function startTab(){
  const fromHash = location.hash.replace('#','');
  const saved = localStorage.getItem('hq:tab') || '';

  const isValid = id => TABS.some(t => t.id === id);

  let target = 'home';

  if (isValid(fromHash)) {
    target = fromHash;
  } else if (isValid(saved)) {
    target = saved;
  } else {
    localStorage.setItem('hq:tab', 'home');
  }
...
function initMap(){
  ...
  try{
    ...
    if (els.mapLegend){
      ...
      els.mapLegend.innerHTML = mk(cs.getPropertyValue('--sale'),'Sale')
        + mk(cs.getPropertyValue('--trade'),'Trade')
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  startTab();   // ✅ this now safely picks home / saved / hash
});
