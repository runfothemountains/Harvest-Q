...
const LANG = {
  ...
  hi: {
    ...
    'mode-all':'सभी','mode-sale':'बिक्री के लिए','mode-ट्रेड':'विनिमय के लिए','mode-both':'दोनों',
...
const els = {
  ...
  consumerMapEl: E('consumer-map'), consumerMapLegend: E('consumer-map-legend'),
...
function initMap(){
  ...
  try{
    ...
    // Hide loading spinner and show map once initialized
    const loading = E('map-loading');
    ...
    if (els.mapLegend){
      ...
      els.mapLegend.innerHTML =
        mk(cs.getPropertyValue('--sale'),'Sale') +
    ...
    const loading = E('map-loading');
    if (loading) loading.textContent = 'Map failed to load.';
  }
}
...
function initConsumerMap(){
  ...
  try{
    consumerMap = L.map('consumer-map',{zoomControl:true}).setView([10,10],2);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:''}).addTo(consumerMap);

    // Hide loading spinner and show map once initialized
    const loading = E('consumer-map-loading');
    if (loading) loading.style.display = 'none';
    els.consumerMapEl.style.display = 'block';

    if (els.consumerMapLegend){
      const mk = (color,label)=>`<div class="item"><div class="dot" style="background:${color}"></div><div class="label">${label}</div></div>`;
      const cs = getComputedStyle(document.documentElement);
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  // Respect the inline footer-date span if present.
  const footerDateSpan = E('footer-date');
  if (!footerDateSpan && els.footerText) {
    els.footerText.textContent = `© Harvest Q — Stage 3 • ${new Date().toLocaleString()}`;
  }
